...
- Live current-set tracking from open iTerm sessions so you can save the in-progress working set as a preset
...
- Relevant-projects-first UI with optional expansion to the full discovered list
...
By default, the UI only shows relevant projects: live sessions, saved presets, recent launches, and previously used rows.
You can expand the list on demand to see every discovered project.

## Product direction

Longer-term product notes live in [docs/VISION.md](docs/VISION.md), including session monitoring, keep-alive/cache-age ideas, and an eventual agent-state dashboard.
